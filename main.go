...
func prepareToProcess() {
	...
	if err != nil {
		log.Println("Failed to open CSV file: %v", err)
	} else {
		infrastructure.DeletePromotionsCollection(client)
		err = Process(file, client)
		if err != nil {
			log.Println("Failed on Process file: %v", err)
		}

		func(file *os.File) {
			err := file.Close()
			if err != nil {
				log.Println("Failed to close CSV file: %v", err)
			}
			err = os.Remove(path + "/promotions.csv")
			if err != nil {
				log.Println("Failed to Remove CSV file: %v", err)
			}
		}(file)
	}
}
